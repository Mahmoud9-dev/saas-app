...
export const getCompanion = async (id: string) => {
  ...
  // If user is logged in, check if companion is bookmarked
  if (userId) {
...
export const getUserSessions = async (userId: string, limit = 10) => {
  ...
  // Check which companions are bookmarked
  const { data: bookmarks, error: bookmarksError } = await supabase
    .from("bookmarks")
    .select("companion_id")
    .eq("user_id", userId);
...
export const getBookmarkedCompanions = async (userId: string) => {
  ...
  // Since these are all bookmarked companions, set bookmarked to true for all of them
